...
#-------------------------------------------------------------------------
# trial5   conv3_netに入力特徴量に加速度(前回と今回の速度の差)を追加したもの
#          separate-inputs move,etc=>MLP and Lidar=>CNN1D  
#          
#
class CONV4_Net(torch.nn.Module):
    def __init__(self, num_inputs, action_space):
        super(CONV4_Net, self).__init__()
        # nn.Conv1d(in_channels, out_channels, kernel_size, strid
        # 10 lindar
        self.dim2= 4 # num_inputs, stack_frame is 4 fixed
        self.conv1 = nn.Conv1d(self.dim2, 32, 3, stride=1, padding=1)
...
#-------------------------------------------------------------------------
# trial6  CONV4_Netに
#         feedback from action-output to lstm-input を追加したもの
#
class CONV5_Net(torch.nn.Module):
    def __init__(self, num_inputs, action_space):
        ...
        #--- feedback from xaction to lstm input
        
        self.alfc1 = nn.Linear(4,32)
        self.lrelu10 = nn.LeakyReLU(0.1)
        self.alfc2 = nn.Linear(32,32)
        self.lrelu11 = nn.LeakyReLU(0.1)
        #self.sigmoid1  = nn.Sigmoid()  # output is from 0 to 1
        
        #---
        self.dim3= 128 + 352 + 32 ## 704
        self.lstm1 = nn.LSTMCell(self.dim3, 128)
        #---
        ...
        self.alfc1.weight.data.mul_(lrelu)
        self.alfc2.weight.data.mul_(lrelu)
    ...
    def forward(self, inputs):
        x, (hx1, cx1, fx1) = inputs # fx1 is feedback from previous action value
        ...
        #-- feedback
        xfb=  self.lrelu10(self.alfc1(fx1))
        xfb=  self.lrelu11(self.alfc2(xfb))
        x3 = xfb.view(1, 32)
        ...
        #--- for feedback
        xaction= F.softsign(self.actor_linear(x))
        fx1=xaction
        
        
        return self.critic_linear(x), xaction, self.actor_linear2(x), (hx1, cx1, fx1)
...
class CONV6_Net(torch.nn.Module):
    ...
    def forward(self, inputs):
        x, (hx1, cx1, hx2, cx2) = inputs  # Currently, hx2 and cx2 is not used
        ...
        return self.critic_linear(x), F.softsign(self.actor_linear(x)), self.actor_linear2(x), (hx1, cx1, hx2, cx2)
